...
<body class="body">
  <!-- navbar -->
  <p id="audio-info-desain">Menunggu data...</p>
  <p id="audio-info-siap">Menunggu data...</p>
    ...
    <div class="row">
      ...
      <script>
        ...
        function playAudio1() {
          ...
          setTimeout(function () { audio4desain.play(); }, 8000);
          setTimeout(function () { audio5desain.play(); }, 9000);
          setTimeout(function () { audio6desain.play(); }, 10000);
          setTimeout(function () { audio7desain.play(); }, 11000);
          setTimeout(function () { audio8desain.play(); }, 14000);
          ...
          $.ajax({
            ...
            success: function (response) {
              if (response.status === 'success') {
                ...
                $('#audio-info-desain').html(audioDataDesain.hurufantriandesain + audioDataDesain.jumlahantriandesain + audioDataDesain.loketantriandesain + audioDataDesain.nomorantriandesain);

                // Set audio sources based on data
                ...
                } else if (audioDataDesain.jumlahantriandesain == 2) {
                  const nomoradesain = audioDataDesain.nomorantriandesain[0];
                  const nomorbdesain = audioDataDesain.nomorantriandesain[1];
                ...
                } else if (audioDataDesain.jumlahantriandesain == 3) {
                  const nomoradesain = audioDataDesain.nomorantriandesain[0];
                  const nomorbdesain = audioDataDesain.nomorantriandesain[1];
                  const nomorcdesain = audioDataDesain.nomorantriandesain[2];
                  document.getElementById('audio4desain').src = `nadav2fix/100.mp3`;
                  document.getElementById('audio5desain').src = `nadav2fix/${nomorbdesain}.mp3`;
                  document.getElementById('audio6desain').src = `nadav2fix/puluh.mp3`;
                  document.getElementById('audio14desain').src = `nadav2fix/${nomorcdesain}.mp3`;
                }
                ...
                $('#audio-info-desain').html('Data audio tidak ditemukan.');
              }
            },
            error: function () {
              $('#audio-info-desain').html('Terjadi kesalahan saat mengambil data.');
            }
          });
        }
      ...
      <script>
        ...
        function playAudio2() {
          ...
          setTimeout(function () { audio4siap.play(); }, 8000);
          setTimeout(function () { audio5siap.play(); }, 9000);
          setTimeout(function () { audio6siap.play(); }, 10000);
          setTimeout(function () { audio7siap.play(); }, 11000);
          setTimeout(function () { audio8siap.play(); }, 14000);
          ...
          $.ajax({
            ...
            success: function (response) {
              if (response.status === 'success') {
                ...
                $('#audio-info-siap').html(audioDatasiap.hurufantriansiap + audioDatasiap.jumlahantriansiap + audioDatasiap.loketantriansiap + audioDatasiap.nomorantriansiap);
                
                document.getElementById('audio3siap').src = `nadav2fix/${audioDatasiap.hurufantriansiap}.mp3`;

                const jumlah = audioDatasiap.nomorantriansiap.length;

                let nomora = null, nomorb = null, nomorc = null;
                if (jumlah === 2) {
                  nomora = audioDatasiap.nomorantriansiap.substring(0, 1);
                  nomorb = audioDatasiap.nomorantriansiap.substring(1, 2);
                } else if (jumlah === 3) {
                  nomora = audioDatasiap.nomorantriansiap.substring(0, 1);
                  nomorb = audioDatasiap.nomorantriansiap.substring(1, 2);
                  nomorc = audioDatasiap.nomorantriansiap.substring(2, 3);
                }

                const audioSources = [];

                if (jumlah === 1) {
                  audioSources.push(`nadav2fix/${audioDatasiap.nomorantriansiap}.mp3`);
                } else if (audioDatasiap.nomorantriansiap === "10") {
                  audioSources.push("nadav2fix/10.mp3");
                } else if (audioDatasiap.nomorantriansiap === "11") {
                  audioSources.push("nadav2fix/11.mp3");
                } else if (audioDatasiap.nomorantriansiap === "100") {
                  audioSources.push("nadav2fix/100.mp3");
                } else if (audioDatasiap.nomorantriansiap === "110") {
                  audioSources.push("nadav2fix/100.mp3");
                  audioSources.push("nadav2fix/10.mp3");
                } else if (audioDatasiap.nomorantriansiap === "111") {
                  audioSources.push("nadav2fix/100.mp3");
                  audioSources.push("nadav2fix/11.mp3");
                } else if (audioDatasiap.nomorantriansiap === "120") {
                  audioSources.push("nadav2fix/100.mp3");
                  audioSources.push(`nadav2fix/${nomorb}.mp3`);
                  audioSources.push("nadav2fix/puluh.mp3");
                } else if (parseInt(audioDatasiap.nomorantriansiap) > 120) {
                  audioSources.push("nadav2fix/100.mp3");
                  audioSources.push(`nadav2fix/${nomorb}.mp3`);
                  audioSources.push("nadav2fix/puluh.mp3");
                  audioSources.push(`nadav2fix/${nomorc}.mp3`);
                } else if (jumlah === 2 && parseInt(audioDatasiap.nomorantriansiap) < 20) {
                  audioSources.push(`nadav2fix/${nomorb}.mp3`);
                  audioSources.push("nadav2fix/belas.mp3");
                } else if (jumlah === 2 && parseInt(audioDatasiap.nomorantriansiap) > 19) {
                  audioSources.push(`nadav2fix/${nomora}.mp3`);
                  audioSources.push("nadav2fix/puluh.mp3");
                  audioSources.push(`nadav2fix/${nomorb}.mp3`);
                } else if (jumlah === 3 && parseInt(audioDatasiap.nomorantriansiap) < 110) {
                  audioSources.push("nadav2fix/100.mp3");
                  audioSources.push(`nadav2fix/${nomorc}.mp3`);
                } else if (jumlah === 3 && parseInt(audioDatasiap.nomorantriansiap) > 111) {
                  audioSources.push("nadav2fix/100.mp3");
                  audioSources.push(`nadav2fix/${nomorc}.mp3`);
                  audioSources.push("nadav2fix/belas.mp3");
                }
                ...
                if (container) {
                  container.innerHTML = ""; // Clear previous audio elements
                  audioSources.forEach((src, index) => {
                    const audio = document.createElement("audio");
                    audio.id = `audio${index + 4}`;
                    audio.src = src;
                ...
                $('#audio-info-siap').html('Data audio tidak ditemukan.');
              }
            },
            error: function () {
              $('#audio-info-siap').html('Terjadi kesalahan saat mengambil data.');
            }
          });
        }
